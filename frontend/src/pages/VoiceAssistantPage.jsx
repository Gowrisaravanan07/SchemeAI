...
// Ask GS — agentic status steps shown during processing
const STATUS_STEPS = [
  { icon: '🎤', text: 'Listening...' },
  { icon: '🧠', text: 'Ask GS is understanding your query...' },
  { icon: '📊', text: 'Analyzing schemes for you...' },
  { icon: '✅', text: 'Ask GS is ready with results' },
];
...
const VoiceAssistantPage = () => {
  ...
  const [conversation, setConversation] = useState([
    { role: 'ai', text: "I'm Ask GS, your government schemes assistant. Just speak to me or type, and I'll help you find schemes, check eligibility, or get help applying! 🙏" }
  ]);
    ...
    >
      <div className="mb-6 flex items-center justify-between">
         <div>
          ...
          <p className="text-slate-500 mt-2">Just speak or type to find schemes, check eligibility, or get help applying.</p>
         </div>
         {/* TTS Toggle */}
         <button
           onClick={() => {
               ...
               <div className={`max-w-[80%] md:max-w-[70%] p-4 rounded-2xl ${msg.role === 'user' ? 'bg-indian-navy text-white rounded-tr-sm' : `${msg.isError ? 'bg-red-50 border-red-200' : 'bg-slate-100 border-slate-200'} text-slate-800 rounded-tl-sm border`}`}>
                 {msg.role === 'ai' && (
                    <div className={`flex items-center gap-2 mb-2 ${msg.isError ? 'text-red-500' : 'text-indigo-600'}`}>
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider">Ask GS</span>
                    </div>
                 )}
