...
const LandingPage = ({ onLoginSuccess, onGetStartedSuccess, onDemoLogin }) => {
  ...
  const handleRunDemo = async () => {
    ...
    setError(null);
    try {
      // Call backend to create demo user and analyze eligibility
      const response = await runDemo();
      
      loadDemoCitizen(
         response.citizen_profile,
         response.eligible_schemes,
         response.total_benefits
      );
      navigate('/dashboard');
    } catch (err) {
      console.error(err);
      setError('Demo Initialization failed. Please make sure all services are running.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <section className="py-24 bg-gradient-to-b from-white to-indian-offwhite" id="live-demo">
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-12">
              <h2 className="text-4xl md:text-5xl font-extrabold text-indian-navy mb-6 tracking-tight">CivicAssist Live Engine</h2>
